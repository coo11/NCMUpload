...
const uploadPlugin = require("./songUpload"); // NCMUMod 1: Correct require path
...
module.exports = async (query, request) => {
  ...
  const filename = query.songFile.name
    ...
    .replace(/\s/g, '')
    .replace(/\./g, '_') */
  let extExp = /\.(mp3|flac|m4a|wav|aac|ape)$/i;
  let ext = query.songFile.name.match(extExp)?.[1] || "mp3";
  const filename = query.songFile.name.replace(extExp, "").trim();
  // NCMUMod 2: Better way to parse filename ↑↑
  query.cookie.os = "pc";
  ...
  const qc = query.custom;
  if (qc && qc.override) {
    // NCMUMod 3: Custom some basic info
    songName = qc.name || songName;
    album = qc.album || album;
    artist = qc.artist || artist;
  }
  ...
  if (res.body.needUpload) {
    console.log(`"${query.songFile.name}" need to be uploaded.`); // NCMUMod 4: Show if the file was matched
